...
Сервер: Ubuntu 24.04, Docker и Compose уже установлены. Пока домена нет: HTTP по публичному IP.
Когда появится домен: **[staging-domain-https.md](staging-domain-https.md)** (сертификат не выпускать заранее).
